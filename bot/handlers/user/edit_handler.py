...
async def edit_user_message(msg: Message, state: FSMContext):
    await msg.answer(text="Выберите что хотите отредактировать", reply_markup=get_edit_reply_keyboard().as_markup())
...
async def edit_name(msg: Message, state: FSMContext):
    ...
    await msg.answer(text="Имя отредактировано", reply_markup=get_edit_reply_keyboard().as_markup())
    await state.set_state(UserStateGroup.edit_state)
...
async def edit_address(msg: Message, state: FSMContext):
    ...
    await msg.answer(text="Адрес отредактирован", reply_markup=get_edit_reply_keyboard().as_markup())
    await state.set_state(UserStateGroup.edit_state)
...
async def edit_wish(msg: Message, state: FSMContext):
    ...
    await msg.answer(text="Предпочтения отредактированы", reply_markup=get_edit_reply_keyboard().as_markup())
    await state.set_state(UserStateGroup.edit_state)
...
async def edit_name_message(msg: Message, state: FSMContext):
    await msg.answer(text="Введите имя", reply_markup=get_edit_reply_keyboard().as_markup())
    await state.set_state(UserStateGroup.edit_name_state)
    logging.info(f"Текущий state = {await state.get_state()} Функция - edit_name")


async def edit_address_message(msg: Message, state: FSMContext):
    await msg.answer(text="Введите адрес", reply_markup=get_edit_reply_keyboard().as_markup())
    await state.set_state(UserStateGroup.edit_address_state)
    logging.info(f"Текущий state = {await state.get_state()} Функция - edit_address")


async def edit_wish_message(msg: Message, state: FSMContext):
    await msg.answer(text="Введите предпочтения", reply_markup=get_edit_reply_keyboard().as_markup())
...
def register_edit_handlers(dp: Dispatcher):
    ...
    dp.message.register(edit_wish_message, F.text == 'Предпочтения', UserStateGroup.edit_state)
